...
const outfolder = path.join(__dirname, 'oly');

if (!fs.existsSync(outfolder)) {
    fs.mkdirSync(outfolder);
}
async function getpags() {
    const res = await axios.get("https://olympusxyz.com/api/series?page=1&direction=asc&type=comic", {
        headers: {
    ...
    const data = res.data;
    const lastp = data.data.series.last_page;
    return lastp;
}
async function downloadData() {
    ...
    for (let i = 0; i < pag.length; i += batch_size) {
        ...
        const promBatch = batchCurrent.map(async (page) => {
            const url = `https://olympusxyz.com/api/series?page=${page}&direction=asc&type=`;
            const refererUrl = `https://olympusxyz.com/api/series?page=${page - 1}&direction=asc&type=`;
    ...
    try {
        ...
        for (const file of files) {
            ...
            if (path.extname(file) === '.json') {
                ...
                if (parsedContent && parsedContent.data && parsedContent.data.series && Array.isArray(parsedContent.data.series.data)) {

                    // 2. Ahora sí recorremos el array correcto
                    parsedContent.data.series.data.forEach(manga => {
                        // Mapeamos los datos exactamente al formato que solicitaste
                        listaMangasFinal.push({
                            "slug": manga.slug,
                            "title": manga.name,
                            "type": manga.type,
                            "url": `https://olympusxyz.com/series/comic-${manga.slug}`,
                            "image": manga.cover
                        });
                    });
                }
        ...
        const outputFile = path.join(__dirname, 'tmp', 'oly.json');
        ...
        console.log("[PROCESS] Eliminando carpeta temporal 'oly'...");
        fs.rmSync(outfolder, { recursive: true, force: true });
        console.log("[SUCCESS] Carpeta temporal eliminada correctamente.");
